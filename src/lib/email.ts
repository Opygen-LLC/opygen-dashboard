...
async function getTransporter() {
    ...
    console.log(
        "⚠️ SMTP settings not fully configured in environment. Creating test account via Ethereal...",
    );
    try {
        ...
        console.error(
            "Failed to create Ethereal SMTP fallback transporter:",
            error,
        );
        throw error;
    }
...
export async function sendEmail({
    ...
    try {
        ...
        console.log(
            `✉️ Email successfully dispatched to ${to} (ID: ${info.messageId})`,
        );
        ...
        if ((transporter.options as any).host === "smtp.ethereal.email") {
            ...
            console.log(
                `🔗 Ethereal Sandbox Email Preview Link: ${previewUrl}`,
            );
