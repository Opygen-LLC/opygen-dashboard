  ...
  function middleware(req) {
    ...
    if (token?.needPasswordChange) {
      if (path !== '/change-password' && !path.startsWith('/api/auth/')) {
        ...
        if (path.startsWith('/api/') && path !== '/api/users/change-password') {
    ...
    // 2. Role restrictions
    if (path.startsWith('/admin-dashboard') || path.startsWith('/api/users/add')) {
      if (token?.role !== 'admin') {
        if (path.startsWith('/api/')) {
    ...
    if (path.startsWith('/dashboard') && path !== '/dashboard/change-password') {
      if (token?.role === 'admin') {
